...
/**
 * Created by LiJunlong on 2016/12/31.
 */
...
public class WaveView extends View {
    ...
    private int mLeveLine;//基准线

    private int mWaveLength;//波长 暂定为view宽度为一个波长
    private int mStrokeWidth;//圆的线宽
    private RectF rectF;//圆环的区域
    private int mWaveHeight;//波峰的高度
    ...
    private void initWaveMove() {
        mLeftWaveMoveLength += mSpeed;//波向右滑动的距离增加speed
        if (mLeftWaveMoveLength > mWaveLength) {
            //当增加到一个波长时恢复到0
            mLeftWaveMoveLength = 0;
        }
        invalidate();//重新draw（）；恢复视图
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (!isMeasure && Math.abs(getMeasuredHeight() - getMeasuredWidth()) < 50) {
            //只计算一次，relateLayout 的时候要绘制两次 加宽高判断
            mViewHeight = getMeasuredHeight();
            mViewWidth = getMeasuredWidth();
            mLeveLine = mViewHeight;//初始化波的准位线 起始位视图的底部
            {
                mLeveLine = mViewHeight * (100 - mProgress) / 100;
                if (mLeveLine < 0)
                    mLeveLine = 0;
            }
            //计算波峰值
    ...
    public void setmProgress(int mProgress) {
        ...
        mLeveLine=(100-mProgress)*mViewHeight/100;
    }
